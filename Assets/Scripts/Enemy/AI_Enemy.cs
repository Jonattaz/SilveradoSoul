...
namespace PudimdimGames{
    ...
    public class AI_Enemy : MonoBehaviour{
        ...
        void Update(){
            distance = Vector3.Distance(Crouch.playerPos, transform.position);
        ...
        void ChasePlayer(){            
               if(distance > distToPlayer){ 
                    ...
                    nav.speed = normalSpeed;
                    transform.LookAt(Crouch.playerPos);
                    nav.SetDestination(Crouch.playerPos);
               }else{
        ...
        void FacePlayer(){
            Vector3 direction = (Crouch.playerPos - transform.position).normalized;
